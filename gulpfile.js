...
var paths = {
  ...
  watch: [
    "site/**/*.{swig,json,scss,js}",
    "styleguide/**/*.{swig,json,scss,js}"
  ]
};
      ...
      data(function(file) {
        ...
        if (fs.existsSync(json_file)) {
          ...
          for (i in sizes) {
            _image_resize(
              file.path,
              sizes[i].height * retina,
              sizes[i].name + retina_name
            );
          }
        }
      })
...
gulp.task("image_optimize", function() {
  return gulp
    .src(paths.image_resize_dest + "/*.png")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(newer(paths.images_dest))
...
gulp.task("image_move", function() {
  return gulp
    .src(paths.image_resize_dest + "/*.{png,jpg,.gif}")
    .pipe(plumber({ errorHandler: onError }))
    .pipe(newer(paths.images_dest))
...
gulp.task("js_sg", function() {
  _js(
    paths.styleguide_js_src,
    paths.styleguide_js_filename,
    paths.styleguide_js_dest
  );
});
...
gulp.task("scss_sg", function() {
  _scss(
    "styleguide/" + paths.styleguide_scss_src,
    paths.styleguide_scss_dest,
    paths.styleguide_html_src
  );
});
...
var _html = function(source, dest) {
  return (
    gulp
      ...
      .pipe(
        gulpIgnore.exclude(function(file) {
          if (
            file.path.indexOf("--") !== -1 ||
            file.path.indexOf("__") !== -1
          ) {
  ...
  return (
    gulp
      ...
      .pipe(
        data(function(file) {
          if (grabJSON) {
            components = file.path.replace(
              "styleguide/components/pages",
              "site/components"
            );
...
gulp.task("swig_sg", function() {
  _swig(
    "styleguide/" + paths.swig_src,
    "styleguide/" + paths.swig_dest,
    paths.styleguide_config_json,
    true
  );
});
...
function makeUL(lst) {
  ...
  for (var i = 0; i < lst.length; i++) {
    if (lst[i].type == "folder") html.push(makeLI(lst[i]));
  }
...
gulp.task("sg_kss", function() {
  kss.traverse(
    "./site/components",
    { mask: "*.scss", markdown: true },
    function(err, styleguide) {
      if (err) throw err;

      fs.writeFileSync(
        "styleguide/kss.json",
        JSON.stringify(styleguide.section())
      );
